...
class ImageSegment():
    def __init__(self):
        ...
        # For independent otsu
        # self.ot_choice = {  1:np.array([[1,1,1],[0,0,1],[1,1,1]]),
        #                     2:np.array([[0,1,0],[0,0,0],[1,1,1]]),
        #                     3:np.array([[0,0,0],[0,0,1],[1,1,1]]),
        #                     }
        # For iterative otsu
        self.ot_choice = {  1:np.array([[1,1,1],[0,1,1],[1,1,1]]),
                            2:np.array([[0,1,1],[0,1,1],[1,1,1]]),
                            3:np.array([[0,1,1],[0,1,1],[1,1,1]]),
                            }
    ...
    def multi_otsu(self, img, type=1, fchoice=np.array([])):
        ...
        if(len(img.shape)==2):      # If only one channel
            ...
            for i in range(img.shape[2]):
                ...
                # cv2.imwrite(('img' + str(3) +'_ot'+str(i+1)+'_rgb.jpg'),(255*thres_img[:,:,i]).astype(np.uint8))
...
# Main function
    def main(self, img_id):
        ...
        res_img = self.multi_otsu(img_hsv,2,self.ot_choice[img_id][0,:]).astype(np.float)
        res_hsv = res_img #store
        cv2.imwrite(('img' + str(img_id) +'_omask_hsv.jpg'),(255*res_img).astype(np.uint8))
        cv2.imwrite(('img' + str(img_id) +'_hsv.jpg'),(res_img[:,:,np.newaxis]*img_rgb).astype(np.uint8))
        # Dilate and erode image to remove holes
        res_img = cv2.morphologyEx(res_img,cv2.MORPH_CLOSE, ker)
        cv2.imwrite(('img' + str(img_id) +'_ed_hsv.jpg'),(res_img[:,:,np.newaxis]*img_rgb).astype(np.uint8))
        ...
        res_img = self.multi_otsu(img_rgb,2,self.ot_choice[img_id][1,:]).astype(np.float)
        res_rgb = res_img #store
        cv2.imwrite(('img' + str(img_id) +'_omask_rgb.jpg'),(255*res_img).astype(np.uint8))
        cv2.imwrite(('img' + str(img_id) +'_rgb.jpg'),(res_img[:,:,np.newaxis]*img_rgb).astype(np.uint8))
        # Dilate and erode image to remove holes
        res_img = cv2.morphologyEx(res_img,cv2.MORPH_CLOSE, ker)
        cv2.imwrite(('img' + str(img_id) +'_ed_rgb.jpg'),(res_img[:,:,np.newaxis]*img_rgb).astype(np.uint8))
        c_img1,c_img2 = self.img_contour(res_img)   # Get contours
        ...
        res_img = self.multi_otsu(img_txt,2,self.ot_choice[img_id][2,:]).astype(np.float)
        res_txt = res_img
        cv2.imwrite(('img' + str(img_id) +'_omask_txt.jpg'),(255*res_img).astype(np.uint8))
        cv2.imwrite(('img' + str(img_id) +'_txt.jpg'),(res_img[:,:,np.newaxis]*img_rgb).astype(np.uint8))
        # Dilate and erode image to remove holes
        #res_img = cv2.morphologyEx(res_img,cv2.MORPH_CLOSE, ker)
        # cv2.imwrite(('img' + str(img_id) +'_ed_txt.jpg'),(res_img[:,:,np.newaxis]*img_rgb).astype(np.uint8))
        # c_img1,c_img2 = self.img_contour(res_img)   # Get contours
        ...
        # On Combined masks
        # Or the results
        res_img = np.logical_or(np.logical_or(res_hsv,res_rgb).astype(np.uint8),res_txt).astype(np.uint8)
        cv2.imwrite(('img' + str(img_id) +'_omask_cmb.jpg'),(255*res_img).astype(np.uint8))
        cv2.imwrite(('img' + str(img_id) +'_cmb.jpg'),(res_img[:,:,np.newaxis]*img_rgb).astype(np.uint8))
        # Dilate and erode image to remove holes
        res_img = cv2.morphologyEx(res_img,cv2.MORPH_CLOSE, ker)
        cv2.imwrite(('img' + str(img_id) +'_ed_cmb.jpg'),(res_img[:,:,np.newaxis]*img_rgb).astype(np.uint8))
        c_img1,c_img2 = self.img_contour(res_img)   # Get contours
